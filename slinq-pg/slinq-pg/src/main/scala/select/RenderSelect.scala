...
import slinq.pg.fn.Agg.AggFn
import slinq.pg.run.RunQuery
import slinq.pg.shape.RowShapeSingle
import slinq.pg.section._
import slinq.pg.render._
...
class RenderSelect[M, R](
  ...
  def asColumn = {
    coll.rowShape match {
      ...
      case _ =>
        throw SlinqError("Subquery is invalid")
    }
  }
  ...
  def asAggregation = {
    firstColumn match {
      case col: AggFn =>
      case _ =>
        throw SlinqError("Subquery column must be an aggregation function")
    }
