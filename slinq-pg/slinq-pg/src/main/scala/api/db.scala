...
object db { 
  ...
  def queryAs[R, T](render: => RenderedQuery[R], transform: R => T): ZIO[SlinqPg, SQLException, List[T]] = {
    for {
      inst    <- SlinqPg.get
  ...
  def queryHeadAs[R, T](render: => RenderedQuery[R], transform: R => T): ZIO[SlinqPg, SQLException, T] = {
    for {
      inst    <- SlinqPg.get
  ...
  def queryHeadOptAs[R, T](render: => RenderedQuery[R], transform: R => T): ZIO[SlinqPg, SQLException, Option[T]] = {
    for {
      inst    <- SlinqPg.get
