...
import slinq.pg.api.{Jsonb, SlinqError, NoArg}
...
trait ValOptConv[T] extends ValConv[Option[T]] {
  ...
  def opt = throw SlinqError("cannot use asOpt on Option column")
}
...
object NoArgConv extends ValConvReg[NoArg] {
  def get(rs: ResultSet, index: Int) = throw SlinqError("cannot read NoArg")
  def opt = throw SlinqError("cannot use asOpt on NoArg")
}
...
trait JsonbRead {
  val read: AnyRef => Jsonb = {
    ...
    case obj: PGobject =>
      Jsonb(obj.getValue)
    case obj =>
      throw SlinqError("Not jsonb [%s]".format(obj.toString))
  } 
}
