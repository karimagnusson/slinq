...
import slinq.pg.api.{Jsonb, NoArg, SlinqError}
...
trait DebugRendered {
  ...
  private val stringValue: Any => String = {
    ...
    case value                => throw SlinqError(s"type not supported [$value]")
  }
  ...
  def printStatement[T](caller: T): T = {
    println(statement)
    caller
  }
  
  def printStatementAndArgs[T](caller: T): T = {
    println(statement)
    println(args.filter(notNoArg))
    caller
  }
  
  def printStatementWithArgs[T](caller: T): T = {
    println(
      statement
        .replace("??", "##")
        .replace("?", "%s")
        .replace("##", "?")
        .format(args.filter(notNoArg).map(stringValue) *)
    )
    caller
  }
}