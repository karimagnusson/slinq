...
trait StringMethods extends TypeMethods[String] with InMethods[String] {
  ...
  def concat(cols: TypeCol[?]*) = ConcatFn((Seq(col) ++ cols).toVector)
  def concatWs(glue: String, cols: TypeCol[?]*) = ConcatWsFn(glue, (Seq(col) ++ cols).toVector)
