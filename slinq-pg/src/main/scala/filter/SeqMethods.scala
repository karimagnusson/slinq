...
import slinq.pg.assign.*
import slinq.pg.filter.types.*
import slinq.pg.shape.CachePart.itemConv
import slinq.pg.fn.types.*
import slinq.pg.api.{Arg, ArgSeq}
...
trait SeqMethods[T] extends TypeMethods[Seq[T]] {
  ...
  def addUnique(value: T) = Add(col, value)
  def addUnique(value: Seq[T]) = Add(col, col.conv.put(value))
  def addUniqueAsc(value: T) = AddAsc(col, value)
  def addUniqueAsc(value: Seq[T]) = AddAsc(col, col.conv.put(value))
  def addUniqueDesc(value: T) = AddDesc(col, value)
  def addUniqueDesc(value: Seq[T]) = AddDesc(col, col.conv.put(value))
...
case class SeqCache[T](col: TypeCol[Seq[T]]) extends TypeCache[Seq[T]] {
  ...
  def addUnique(arg: Arg) = CacheSeqAdd(col, itemConv(col.conv))
  def addUnique(arg: ArgSeq) = CacheSeqAdd(col, col.conv)
  def addUniqueAsc(arg: Arg) = CacheSeqAddAsc(col, itemConv(col.conv))
  def addUniqueAsc(arg: ArgSeq) = CacheSeqAddAsc(col, col.conv)
  def addUniqueDesc(arg: Arg) = CacheSeqAddDesc(col, itemConv(col.conv))
  def addUniqueDesc(arg: ArgSeq) = CacheSeqAddDesc(col, col.conv)
}
