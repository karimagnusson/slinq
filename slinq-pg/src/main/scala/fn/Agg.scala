...
object Agg {
  ...
  case class AggSumBigDecimalFn(col: TypeCol[?]) extends BigDecimalCol with SingleColFn with AggFn {
    val template = "sum(%s)::numeric"
  }

  case class AggAvgBigDecimalFn(col: TypeCol[?]) extends BigDecimalCol with SingleColFn with AggFn {
    val template = "avg(%s)::numeric"
  }

  case class AggSumFloatFn(col: TypeCol[?]) extends FloatCol with SingleColFn with AggFn {
    val template = "sum(%s)"
  }

  case class AggSumDoubleFn(col: TypeCol[?]) extends DoubleCol with SingleColFn with AggFn {
    val template = "sum(%s)"
  }

  case class AggSumAsLongFn(col: TypeCol[?]) extends LongCol with SingleColFn with AggFn {
    val template = "sum(%s)"
  }

  case class AggAvgAsDoubleFn(col: TypeCol[?]) extends DoubleCol with SingleColFn with AggFn {
    val template = "avg(%s)"
  }
}
