...
case class ConcatFn(cols: Vector[TypeCol[?]]) extends StringFn {
  def name = "concat"
  val template = s"concat(%s)".format(Vector.fill(cols.size)("%s").mkString(", "))
}

case class ConcatWsFn(glue: String, cols: Vector[TypeCol[?]]) extends StringFn {
  def name = "concat"
