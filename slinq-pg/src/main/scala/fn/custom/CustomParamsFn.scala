...
sealed trait CustomParamsFn {
  ...
  def render(prefix: Prefix) = try {
    template.format(cols.map(_.render(prefix)) *)
  } catch {
    case _ : Throwable => throw SlinqError(s"template and columns do not match [$template]")
  }

  def args = { 
    def loop(chars: List[Char], cols: List[TypeCol[?]], args: List[Any], res: Vector[Seq[Any]]): Vector[Any] = {
      chars match {
    ...
    try {
      ...
      case _ : Throwable => throw SlinqError(s"template and function args do not match [$template]")
    }
  }
}
