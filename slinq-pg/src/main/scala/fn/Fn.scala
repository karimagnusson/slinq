...
object Fn {
  ...
  def concat(cols: TypeCol[?]*) = ConcatFn(cols.toVector)

  def concatWs(glue: String, cols: TypeCol[?]*) = ConcatWsFn(glue, cols.toVector)
  ...
  def json(cols: Seq[Tuple2[String, TypeCol[?]]]) = JsonbResFn(cols)
