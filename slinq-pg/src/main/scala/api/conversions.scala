...
import slinq.pg.filter.*
import slinq.pg.sorting.Sorting
import slinq.pg.assign.Assign
import slinq.pg.update.RenderUpdate
import slinq.pg.delete.RenderDelete
import slinq.pg.insert.{RenderInsert, Values}
import slinq.pg.select.*
import slinq.pg.render.*

trait conversions extends filters {
  ...
  given kzRenderSelect[R]: Conversion[RenderSelect[?, R], RenderedQuery[R]] = x => x.render
  given kzRenderSubquery[R]: Conversion[RenderSelect[?, R], Subquery[R]] = x => x.asSubquery
  ...
  given kzRenderAggregation: Conversion[RenderSelect[?, ?], AggregationSubquery] = x => x.asAggregation
  given kzRenderUpdate: Conversion[RenderUpdate[?], RenderedOperation] = x => x.render
  given kzRenderDelete: Conversion[RenderDelete[?], RenderedOperation] = x => x.render
  given kzRenderInsert: Conversion[RenderInsert, RenderedOperation] = x => x.render
  given kzRenderValues: Conversion[Values[?], RenderedOperation] = x => x.render
  ...
  given kzTypeColToSeq: Conversion[TypeCol[?], Seq[TypeCol[?]]] = (x: TypeCol[?]) => Seq(x)
  ...
  given tupleToProduct[T <: Product](using m: Mirror.ProductOf[T]): Conversion[m.MirroredElemTypes, T] =
    m.fromTuple(_)
  ...
  extension (sc: StringContext) {
    def pgrsql(args: Any*): RawSQLStatement = rsql(args *)
