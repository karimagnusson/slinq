...
import slinq.pg.run.RunOperation
import slinq.pg.render.RenderedOperation
...
class Values[M <: Model](
  ...
  def whereNotExists(pick: M => Seq[TypeCol[?]]) = new RenderInsert(
  ...
  def onConflictOnColumn(pick: M => TypeCol[?]) = new DoUpdate(
    builder,
    pick(builder.model)
  )
}
