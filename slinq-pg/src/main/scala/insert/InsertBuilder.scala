...
import slinq.pg.api.{Model, CacheArg}
import slinq.pg.column.{ModelCol, TypeCol}
import slinq.pg.assign.ValueEq
import slinq.pg.api.SlinqError
import slinq.pg.section.WhereSec
import slinq.pg.model.ModelTable
import slinq.pg.shape.ParamShape
import slinq.pg.section.*
import slinq.pg.render.{Prefix, Renderable, SectionCollector}
...
case class InsertBuilder[M <: Model, P](
  ...
  def whereNotExistsReuse(uniqueCols: Vector[TypeCol[?]]) = {
    new ParamConvReuse(
      paramShape.conv,
  ...
  def onConflictColDoUpdateReuse(updateCols: Vector[TypeCol[?]]) = {
    new ParamConvReuse(
      paramShape.conv,
...
trait BuilderMethods[M <: Model] {
  ...
  def returning(returningCols: Vector[TypeCol[?]]) = {
    SectionCollector(prefix, Vector(
      InsertIntoSec(ModelTable(model)),
      InsertColumnsSec(cols),
      InsertValuesSec(values),
  ...
  def whereNotExists(uniqueCols: Vector[TypeCol[?]]) = {
    SectionCollector(prefix, Vector(
      InsertIntoSec(ModelTable(model)),
      InsertColumnsSec(cols),
  ...
  def onConflictColDoNothing(conflictCol: TypeCol[?]) = {
    SectionCollector(prefix, Vector(
      InsertIntoSec(ModelTable(model)),
      InsertColumnsSec(cols),
      InsertValuesSec(values),
  ...
  def onConflictColDoUpdate(conflictCol: TypeCol[?], updateCols: Vector[TypeCol[?]]) = {
    SectionCollector(prefix, Vector(
      InsertIntoSec(ModelTable(model)),
      InsertColumnsSec(cols),
      InsertValuesSec(values),
  ...
  private def upsert(conflictCol: TypeCol[?], updateCols: Vector[TypeCol[?]]) = {
    
    updateCols.map {
      case col: ModelCol =>
      case _ => throw SlinqError("update columns cannot be a function")
    }

    if (updateCols.isEmpty) {
      throw SlinqError("no update columns selected")
    }

    if (updateCols.contains(conflictCol)) {
      throw SlinqError("cannot update the conflicting column")
    }
  ...
  private def conditional(uniqueCols: Vector[TypeCol[?]]) = {
