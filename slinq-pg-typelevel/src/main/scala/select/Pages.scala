...
package slinq.pg.typelevel.select

import cats.effect.IO
import slinq.pg.typelevel.api.db
...
class Pages[R](query: RenderedQuery[R], limit: Int) {
  ...
  def nextOpt: IO[Option[List[R]]] =
    if (isDone) IO.pure(None)
    else db.query(nextQuery).map(rows => markDone(Some(rows)))

  def next: IO[List[R]] = nextOpt.map {
    case Some(rows) => rows
  ...
  def pageOpt(num: Int): IO[Option[List[R]]] = page(num).map {
    case Nil => None
    case rows => Some(rows)
  }

  def page(num: Int): IO[List[R]] =
