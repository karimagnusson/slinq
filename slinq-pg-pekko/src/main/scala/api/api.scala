...
import slinq.pg.pekko.select.Pages
...
package object api extends conversions {
  ...
  extension [M, R](o: Offset[M, R]) {
    ...
    def source(size: Int = 1000)(implicit ec: ExecutionContext): Source[R, NotUsed] = {
      val pages = asPages(size)
      Source
        .unfoldAsync(pages) { p =>
          p.nextOpt.map {
            case Some(rows) if rows.nonEmpty => Some((p, rows))
            case _ => None
          }
        }
        .mapConcat(identity)
    ...
    def sourceAs[T](size: Int = 1000)(
      using transform: R => T
    )(implicit ec: ExecutionContext): Source[T, NotUsed] =
      source(size).map(transform)
  }
  ...
  extension [P](si: StoredInsert[P]) {
    ...
    def asSink(implicit ec: ExecutionContext): Sink[P, Future[Done]] =
      Sink.foreachAsync[P](1)(params => db.exec(si.render(params)))

    def asListSink(implicit ec: ExecutionContext): Sink[Seq[P], Future[Done]] =
      Sink.foreachAsync[Seq[P]](1)(paramList => db.execList(paramList.map(si.render(_))))
  ...
  extension [P1, P2](su: StoredUpdate[P1, P2]) {
    ...
    def asSink(implicit ec: ExecutionContext): Sink[Tuple2[P1, P2], Future[Done]] =
      Sink.foreachAsync[Tuple2[P1, P2]](1)(p => db.exec(su.render(p._1, p._2)))

    def asListSink(implicit ec: ExecutionContext): Sink[Seq[Tuple2[P1, P2]], Future[Done]] =
      Sink.foreachAsync[Seq[Tuple2[P1, P2]]](1)(list => db.execList(list.map(p => su.render(p._1, p._2))))
  ...
  extension [P](sd: StoredDelete[P]) {
    ...
    def asSink(implicit ec: ExecutionContext): Sink[P, Future[Done]] =
      Sink.foreachAsync[P](1)(params => db.exec(sd.render(params)))

    def asListSink(implicit ec: ExecutionContext): Sink[Seq[P], Future[Done]] =
      Sink.foreachAsync[Seq[P]](1)(paramList => db.execList(paramList.map(sd.render(_))))
